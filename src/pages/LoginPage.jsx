...
export default function LoginPage() {
  ...
  async function handleLogin(e) {
    ...
    try {
      const { token } = await apiLogin(correo, password);

      // ahora login recibe sólo el token y el contexto decodifica
      login(token);
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center",
      ...
      <div style={{ backgroundColor: "white", padding: "40px", borderRadius: "12px",
                    boxShadow: "0 8px 24px rgba(0,0,0,0.15)", width: "100%", maxWidth: "400px" }}>
        ...
        <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <button type="submit" disabled={loading}
                  style={{ padding: "14px", borderRadius: "8px", border: "none", backgroundColor: "#1a3a6d",
                           color: "white", fontWeight: "bold", fontSize: "1.1rem", cursor: "pointer" }}>
            {loading ? "Ingresando..." : "Ingresar"}
          </button>
        </form>

        {error && <p style={{ color: "red", textAlign: "center", marginTop: "15px", fontWeight: "500" }}>{error}</p>}
